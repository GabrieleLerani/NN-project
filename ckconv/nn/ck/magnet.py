...
class AnisotropicGaborLayer(nn.Module):
    def __init__(
        ...
        self.gamma_x = nn.Parameter(
            torch.distributions.gamma.Gamma(
                alpha / (current_layer + 1), beta
            ).sample((hidden_channels,1))
        )
        ...
        normal_dist = torch.distributions.normal.Normal(0, 1)  # Create a normal distribution with mean 0 and std 1
        self.mi_x = nn.Parameter(normal_dist.sample((hidden_channels, data_dim)))
        self.mi_y = nn.Parameter(normal_dist.sample((hidden_channels, data_dim)))
        ...
        self.linear.weight.data *= (
            self.gamma_x.view(*self.gamma_x.shape, *((1,) * data_dim))
        )
    ...
    def forward(self, x):
        ...
        reshaped_gamma_x = self.gamma_x.view(*self.gamma_x.shape, *((1,) * self.data_dim))
        reshaped_gamma_y = self.gamma_y.view(*self.gamma_y.shape, *((1,) * self.data_dim))
        
        reshaped_mi_x = self.mi_x.view(*self.mi_x.shape, *((1,) * self.data_dim)) 
        reshaped_mi_y = self.mi_y.view(*self.mi_y.shape, *((1,) * self.data_dim)) 

        g_envelope = torch.exp(
            -0.5
            * (
                (reshaped_gamma_x * (reshaped_x - reshaped_mi_x)) ** 2
                + (reshaped_gamma_y * (reshaped_y - reshaped_mi_y)) ** 2
            )
        )
        ...
        print(f"sinusoidal shape: {sinusoidal.shape}")
...
if __name__ == "__main__":
    # test the model
    data_dim = 2
    model = MAGNet(data_dim=data_dim, hidden_channels=32, out_channels=10, no_layers=3)
